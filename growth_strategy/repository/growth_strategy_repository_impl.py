...
class GrowthStrategyRepositoryImpl(GrowthStrategyRepository):
    ...
    async def fetch_growth_strategy(self, content_categories, ages, genders, visibility, platforms, investment_amount, upload_frequency, interested_influencer, userToken, request_id):
        ...
        prompt = PromptTemplate.from_template("""
                         ...
                         ref: 
                         ### 입력 요약 - {{컨텐츠}}: 뷰티 - {{나이대}}: 10대 - {{성별}}: 남자 - {{공개 정도}}: 얼굴 공개 - {{플랫폼}}: YouTube - {{투자 가능 금액}}: 없다 - {{업로드 가능 주기}}: 매일 - {{관심 있는 인플루언서}}: 레오제이 
                         ...
                         ### 인플루언서 분석: {{관심 있는 인플루언서}} - {{관심 있는 인플루언서}}는 {{콘텐츠}} 유튜버로서 재치 있는 진행과 카리스마 있는 메이크업 스타일로 인기를 끌고 있습니다. - 다양한 {{콘텐츠}} 콘텐츠(메이크업 튜토리얼, 일상 브이로그)를 통해 시청자와 친근하게 소통하며, 자신의 스타일을 확고하게 유지한 것이 성공 요인입니다. 적용 전략: - {{관심 있는 인플루언서}}처럼 캐릭터와 자신감 있는 태도를 유지하며 시청자와 소통하세요. - 초기에는 간단한 학생용 스킨케어 팁이나 저가 메이크업 튜토리얼로 시작하고, 점차 본인만의 {{콘텐츠}} 스타일을 찾아가는 것이 좋습니다. - {{관심 있는 인플루언서}}처럼 유머와 스토리텔링을 더해 시청자와 친밀감을 쌓는 것이 중요합니다. --- 
                         ### 콘텐츠 전략 1. 기초 스킨케어 및 저예산 화장품 리뷰 - 학생들에게 맞는 저렴한 스킨케어 제품 리뷰와 여드름 관리법을 소개합니다. - “10대가 꼭 알아야 할 {{콘텐츠}} 꿀팁”과 같은 타이틀로 시청자들의 관심을 유도합니다. 2. 데일리 메이크업 챌린지 - 일주일 동안 매일 다른 테마로 메이크업을 시도하는 챌린지 콘텐츠를 기획합니다. - 예: "학교 가기 전 5분 메이크업," "K-팝 아이돌 룩 도전하기." 3. {{콘텐츠}} 관련 브이로그 - 자신의 일상 속에서 화장품을 사용하는 모습을 담은 브이로그를 통해 친근한 이미지를 구축합니다. - "화장품 구매 브이로그" 또는 "메이크업 실습일기"와 같은 콘텐츠가 유효합니다. 4. 유머와 스토리텔링 활용 - {{관심 있는 인플루언서}}처럼 재미있는 일화나 에피소드를 곁들여 콘텐츠를 만듭니다. 시청자들이 쉽게 공감할 수 있도록 유머러스한 진행을 시도해보세요. --- 
                         ### 예산별 장비 및 툴 추천 - {{투자 가능 금액}}에 맞는 장비들을 추천합니다. - 예: "Adobe-Pro" (10만원) --- 
                         ### 총정리: 성장 로드맵 1. 1~2개월: 저예산 스킨케어 및 메이크업 제품 리뷰로 시작하여 구독자 기반을 다집니다. 2. 3~6개월: {{업로드 가능 주기}} 업로드를 유지하며, 다양한 메이크업 챌린지에 도전합니다. 3. 6개월~1년: 댓글과 라이브 방송을 활용해 소통을 강화하고, 새로운 트렌드와 협업 기회를 모색합니다. 4. 1년 후: 본인만의 {{콘텐츠}} 스타일을 확립하고, 다른 {{플랫폼}} 인플루언서와의 콜라보 콘텐츠로 새로운 팬층을 유입시킵니다. --- 
                         ### 응원 메시지 🎉 {{콘텐츠}} 유튜버의 길에 도전하는 당신을 응원합니다! {{관심 있는 인플루언서}}처럼 자신감 있는 태도로 새로운 {{콘텐츠}} 트렌드를 이끌어가세요. 꾸준한 노력과 진솔한 소통이 당신의 가장 큰 무기가 될 것입니다. 당신의 성장이 곧 1 {{나이대}} {{성별}} {{콘텐츠}} 시장의 새로운 기준이 될 거예요! 
                       """)
        try:
